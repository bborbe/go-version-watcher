...
It is sent to the `{branch}-core-notification-v1-request` topic (CDB, `NotificationV1SchemaID`); the notification core's controller routes `go-release` to Discord. The message never carries a target — per [[Notification Targets]], the message does not know where it ends.
